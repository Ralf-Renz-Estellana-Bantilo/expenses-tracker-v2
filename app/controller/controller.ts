...
export const fetchPastWeekExpense = async <T> ( payload: { user: string } ): Promise<T> => {
   return await fetchURL( URL_PASTWEEKEXPENSES, payload );
}

export const fetchMonthlyPercentageBreakdown = async <T> ( payload: { user: string } ): Promise<T> => {
   return await fetchURL( URL_MONTHLYPERCENTAGEBREAKDOWN, payload );
}

export const fetchDailyExpenses = async <T> ( payload: { user: string } ): Promise<T> => {
   return await fetchURL( URL_DAILYEXPENSES, payload );
}

export const fetchMonthlyExpenses = async <T> ( payload: { user: string } ): Promise<T> => {
   return await fetchURL( URL_MONTHLYEXPENSES, payload );
}

export const fetchSummary = async <T> ( payload: { user: string } ): Promise<T> => {
