...
const CustomizedLabel = ({ x, y, stroke, value }) => {
  return (
    <text x={x} y={y} dy={-4} fill={stroke} fontSize={10} className='text-white' textAnchor="middle" color='white'>
      {value}
    </text>
  );
};
...
const CustomizedLabelLineChart = () => {
   ...
   const getPastWeekExpenses = async() => {
      ...
      const uniqueCategory = [...new Set(response.map(({categoryID}) => categoryID))]
    ...
    <ResponsiveContainer width="100%" height="100%">
      ...
      >
        ...
        {categories.map((category) => {
         return <Line type="monotone" dataKey={category.categoryID} stroke={setRandomColor(category.categoryID)}/>
        })}
      </LineChart>
    </ResponsiveContainer>
  );
