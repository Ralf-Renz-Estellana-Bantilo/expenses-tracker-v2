...
const TodaysExpenses = () =>
{
   ...
   {
      const result = context?.todayExpenses?.reduce( ( accumulator, item ) => Number( accumulator ) + Number( item.amount ), 0 ) ?? 0
      ...
      <>
         ...
         <Wrapper>
            <WrapperHeader className='flex items-center justify-between'>
               ...
               {totalBalance > 0 && <Button isIconOnly color="primary" size='sm' variant="light" aria-label="Take a photo" onPress={() => showExpenseDialog( null )}>
                  <PlusIcon />
               </Button>}
