...
const CategoryPercentageAnalytics = () =>
{
   ...
   const [percentageBreakdown, setPercentageBreakdown] = useState<AnalyticsPercentageType[]>( [] )
   ...
   const getMonthlyPercentageBreakdown = async () =>
   {
      if ( user )
      {
         try
         {
            const response = await fetchMonthlyPercentageBreakdown( { user } ) as AnalyticsPercentageType[]
   ...
   return (
      <div className="flex flex-col p-3 border-1 border-border-color rounded-lg gap-2">
         <div className="flex items-center justify-between">
            <h3 className='text-center font-semibold text-accent-primary'>Category Percentage Breakdown <code className='font-normal'>({getCurrentMonth()})</code></h3>
            <Button isIconOnly color="default" variant="light" aria-label="Take a photo">
               <EllipsisVertical />
            </Button>
         </div>
         <div className="flex flex-col gap-3">
            {isPending.current ? <div className="flex flex-col gap-2">
               <div className="flex items-center justify-between">
               ...
               percentageBreakdown.map( data => (
                  <div className="flex flex-col" key={data.categoryID}>
                     <div className="flex justify-between">
                        ...
                        <span className='text-sm text-accent-secondary'>{data.percentage}%</span>
                     </div>
                     <div className="flex rounded-md overflow-hidden">
                        <div className={`h-2 bg-slate-500`} style={{ width: `${data.percentage}%`, backgroundColor: `${setRandomColor( data.categoryID )}` }} />
                        <div className={`w-full h-2`} style={{ backgroundColor: `${setRandomColor( data.categoryID )}`, opacity: .1 }} />
                     </div>
                  </div>
               ) )
            }</>}
